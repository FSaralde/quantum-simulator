...
This folder also includes a Vercel serverless API entrypoint at `api/index.py`.
The `pyproject.toml` file points Vercel to that entrypoint.

In Vercel, deploy this folder:

```text
outputs/quantum_web_app
```
...
The Python file `backend.py` is for local/Render-style servers. On Vercel, `api/index.py` is the backend entrypoint and it routes `/api/chat`, `/api/health`, `/api/visual`, and the other API URLs.
