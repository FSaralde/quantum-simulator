...
DATA_FILE = Path(os.environ.get("APP_DATA_FILE", ROOT / "app_data.json"))
...
class QuantumHandler(BaseHTTPRequestHandler):
    ...
    def send_auth_json(self, payload: dict[str, object], sid: str | None = None, clear: bool = False, status: int = 200) -> None:
        ...
        secure_cookie = self.headers.get("Origin", "").startswith("https://")
        same_site = "None" if secure_cookie else "Lax"
        secure = "; Secure" if secure_cookie else ""
        if sid:
            self.send_header("Set-Cookie", f"{SESSION_COOKIE}={sid}; Path=/; SameSite={same_site}; HttpOnly{secure}; Max-Age=1209600")
        if clear:
            self.send_header("Set-Cookie", f"{SESSION_COOKIE}=; Path=/; SameSite={same_site}; HttpOnly{secure}; Max-Age=0")
        self.end_headers()
    ...
    def send_cors_headers(self) -> None:
        configured_origin = os.environ.get("CORS_ORIGIN", "").strip()
        request_origin = self.headers.get("Origin", "").strip()
        allowed_origin = configured_origin if configured_origin and configured_origin != "*" else request_origin
